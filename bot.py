...
def load_memory():
    if os.path.exists(MEMORY_FILE):
        with open(MEMORY_FILE, "r") as f:
            data = json.load(f)
            # Migrate old patterns to new schema
...
# ===== LIVE MARKET PRICES =====
try:
    nvda_price = float(yf.Ticker("NVDA").history(period="1d")['Close'].iloc[-1])
    qqq_price = float(yf.Ticker("QQQ").history(period="1d")['Close'].iloc[-1])
    btc_price = float(yf.Ticker("BTC-USD").history(period="1d")['Close'].iloc[-1])
    eth_price = float(yf.Ticker("ETH-USD").history(period="1d")['Close'].iloc[-1])
except:
...
tech_symbols = ['NVDA', 'QQQ', 'AAPL', 'MSFT', 'AMD', 'TSM', 'AVGO', 'SMH']
...
if len(memory["patterns"]) >= 5:
    prices = [p.get('nvda', nvda_price) for p in memory["patterns"][-5:]]
    slope = (prices[-1] - prices[0]) / prices[0] * 100 if prices[0] else 0
    if slope > 1: trend = "UP"
...
if tech_exposure > 40:
    ...
    shares_to_trim = int(excess / nvda_price) if nvda_price else 0
...
if token and chat_id:
    ...
    equities = [p.get('equity', equity) for p in memory['patterns']]
    plt.plot(equities, linewidth=2.5, marker='o', markersize=3)
